...
import {getSocketByChannel, subscribeToChannel} from "../registry/channel.registry.js"
...
import {connectionRegistry} from "../registry/connection.registry.js"
...
export function handleMessage(connectionId:string, raw: string){
    ...
    try{
        ...
        connectionRegistry.get(connectionId)?.ws.close(WS_CLOSE.INVALID_MSG, "invalid json") 
        return
    }
    ...
    if(!isInboundMessage(parsed)){
        ...
        connectionRegistry.get(connectionId)?.ws.close(WS_CLOSE.INVALID_MSG, "invalid message")
        return
    }
    ...
    switch(parsed.type){
        case "subscribe": {
            ...
            subscribeToChannel(connectionId, parsed.roomId)
            break
        }
        
        case "chat": {
            if(!parsed.roomId || !parse.text){
        ...
        case "replay_request": {
            if (!parsed.roomId || !parsed.fromId) {
                logger.warn({ connectionId }, "replay_request missing roomId or fromId");
                return;
            }
        ...
        default: {
            ...
            logger.warn({ connectionId, type: (_exhaustive as { type: string }).type }, "Unhandled message type");
        }
    }
}

export async function startSubscriber() {
    await sub.pSubscribe("*", (message: string, channel: string) => {
        const sockets = getSocketByChannel(channel)
