...
function requireSubscriber(): RedisClientType {
  if (!_subscriber) {
    throw new Error(
      "ChannelRegistry not initialised, call initChannelRegistry first",
    );
  }
  return _subscriber;
}
...
export async function joinChannel(
  ...
  if (members.get(channel)?.has(connId)) {
    logger.debug(
      { connId, roomId },
      "joinChannel called but already subscribed",
    );
    return;
  }
...
export async function leaveChannel(
  ...
  if (!members.get(channel)?.has(connId)) {
    logger.warn(
      { connId, roomId },
      "leaveChannel called but connection not in channel",
    );
    return;
  }
  ...
  if (next <= 0) {
    ...
    logger.debug(
      { channel, roomId },
      "Unsubscribed from Redis channel — no local subscribers remain",
    );
  } else {
